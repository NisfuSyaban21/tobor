...
void controlMotor(int pinA, int pinB, int pinPWM, float speed) {
  ...
  if (ishoming) {
    if (siklus_homing == 0) { pwmValue = min(55, max(pwmValue, 35)); }
    else if (siklus_homing == 1) { pwmValue = min(50, max(pwmValue, 10)); }
    else if (siklus_homing >= 2) { pwmValue = min(60, max(pwmValue, 0)); }
  } else {
    if (siklus_home == 0) { pwmValue = min(45 ,max(pwmValue, 35)); }
    else if (siklus_home == 1) { pwmValue = min(37, max(pwmValue, 10)); }
    else if (siklus_home == 2) { pwmValue = min(45 ,max(pwmValue, 37)); }
    else if (siklus_home == 3) { pwmValue = min(37, max(pwmValue, 10)); }
    else if (siklus_home == 4) { pwmValue = min(45, max(pwmValue, 37)); }
    else if (siklus_home == 5) { pwmValue = min(45, max(pwmValue, 37)); }
...
int stepsilo1(){
  ...
  long erhomy_2 = distance1 - spy;
  long averhomy = ((distance1 + distance2)/2) - spy;
  long erromega = (distance2 - distance1);
  
  moveRobot(erhomx,erhomy_1,0);
  ...
  long distance3 = readUltrasonic(trigPin3, echoPin3);  // samping
  long distance4 = readUltrasonic(trigPin4, echoPin4);
  long distance5 = readUltrasonic(trigPin5, echoPin5);  // samping
  ...
  long erhomy_1 = distance2 - spy;
  long erhomy_2 = distance1 - spy;
  long erhomminy = (distance4 - spy)*-1;
  long averhomyy = (((distance4 + distance5)/2) - spy)*-1;
  long erromega = (distance2 - distance1);
  
  moveRobot(erhomx,averhomyy,0);
  if (millis() >= time_now + interval){
    time_now += interval;
  if ((erhomx >= -1 && erhomx <= 1)&&(erhomminy >= -1 && erhomminy <=1 )){
  moveRobot(0,0,0);
