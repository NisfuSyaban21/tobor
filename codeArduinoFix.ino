...
const int echoPin6 = 48;

const int trigPin7 = 50;
const int echoPin7 = 52;
...
const int encA4 = 21;//A4
const int encB4 = 42;//B4
const int encA3 = 20;//A3
const int encB3 = 40;//B3
const int encA1 = 18;// A1
const int encB1 = 34;// B1
const int encA2 = 19;//A2
const int encB2 = 36;//B2
...
float Ki = 0.6;  // Koefisien integral
...
float maxRPM = 6.0;  // RPM maksimum yang diizinkan
float minRPM = 1.0;  // RPM minimum yang diizinkan
...
void moveMotorByPID(float desiredRPM, float actualRPM, int pinA, int pinB, int pinPWM, float* previousError, float* integral, float omega) {
  ...
  *previousError = error;  // Simpan error sebelumnya untuk perhitungan berikutnya
  ...
  pwm = constrain(pwm, 0, 70);  // Batasi nilai PWM antara 0 dan 255
...
void loop() {
  ...
  long distance7 = readUltrasonic(trigPin7, echoPin7);  // samping
  ...
  if (!isGerakkankeRunning && a==0 && b==0) { //tambahin kondisi sensor warna dan ultra
    ...
    Serial.println("can kadetek a");
    }
      
    }
    else {
    Serial.println("no bola");
    }
    delay(50);
    }
  } 
...
int stepsilo1() {
    ...
    if (millis() - lastTime >= 100) {
      long distance1 = readUltrasonic(trigPin1, echoPin1);  // kiri
      long distance2 = readUltrasonic(trigPin2, echoPin2);  // Kanan
      long distance3 = readUltrasonic(trigPin3, echoPin3);  // Kanan
      lastTime = millis();
      ...
      if ((erhomx >= -1 && erhomx <= 1)&&(erhomy_1 >= -1 && erhomy_1 <= 1)) {  // Ketika berada dalam jarak target
    ...
    if (millis() - lastTime >= 100) {
      ...
      long distance2 = readUltrasonic(trigPin2, echoPin2);  // Kanan
      long distance3 = readUltrasonic(trigPin3, echoPin3);  // Kanan
      long distance5 = readUltrasonic(trigPin5, echoPin5);  // Kanan
      lastTime = millis();
      Serial.print("distance belakang:");
      Serial.println(distance5);
      
      long erhomw = (distance1 - distance2);

      float V_w = K_p * erhomw;
      ...
      // Kondisi untuk berhenti
      if (erhomw >= -1 && erhomw <= 1) {  // Ketika berada dalam jarak target
        // Hentikan robot dengan memanggil `moveRobot` dengan semua kecepatan 0
        moveRobot(0, 0, 0);
        delay(1000);
        siklus_home = 2;
    ...
    if (millis() - lastTime >= 100) {
      ...
      long distance3 = readUltrasonic(trigPin3, echoPin3);  // Kanan
      long distance5 = readUltrasonic(trigPin5, echoPin5);  // Kanan
      long distance4 = readUltrasonic(trigPin4, echoPin4);  // Kanan
      lastTime = millis();

      
      long erhomw = (distance4 - distance5);
      Serial.print("distance belakang:");
      Serial.println(erhomw);
...
int stepsilo4() {
    ...
    if (millis() - lastTime >= 100) {
      ...
      long distance2 = readUltrasonic(trigPin2, echoPin2);  // Kanan
      long distance3 = readUltrasonic(trigPin3, echoPin3);  // Kanan
      long distance6 = readUltrasonic(trigPin6, echoPin6);  // Kanan
      lastTime = millis();
      Serial.print("distance depan:");
      Serial.println(distance1);
